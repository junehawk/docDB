...
- Python 3.10+
- macOS / Linux / Windows
- **RAM 8GB 이상** (16GB+ 권장, 아래 메모리 가이드 참고)
...
docDB는 로컬 임베딩 모델(bge-m3, ~2GB)과 Reranker 모델(bge-reranker-v2-m3, ~1GB)을 사용합니다.
인덱싱 시 문서 추출 + 임베딩 + 저장이 동시에 일어나므로, 시스템 메모리가 충분해야 합니다.

### 장비별 권장 사항

| RAM | 인덱싱 가능 규모 | 비고 |
|-----|------------------|------|
| 8GB | ~100파일 | 다른 앱 최소화 권장, swap 발생 가능 |
| 16GB | ~500파일 | 일반 사용에 충분 |
| 32GB | ~2,000파일 | 쾌적한 사용 |
| 64GB+ | 5,000파일+ | 대규모 인덱싱 가능 |

### 인덱싱 시 메모리 사용량 (실측)

| 항목 | 사용량 |
|------|--------|
| 임베딩 모델 (bge-m3) | ~2GB (GPU/MPS 메모리) |
| Reranker 모델 | ~1GB (검색 시에만) |
| 인덱싱 파이프라인 | ~0.5-1.5GB (문서 크기에 비례) |
| **인덱싱 피크 합계** | **~3-5GB** |

### Apple Silicon (MPS) 사용자 참고

Apple Silicon Mac에서는 PyTorch MPS 백엔드가 메모리를 적극적으로 캐싱합니다.
Activity Monitor에서 메모리가 높게 표시될 수 있지만, docDB는 내부적으로 다음 최적화를 적용하고 있습니다:

- MPS 메모리 즉시 반환 모드 (`PYTORCH_MPS_HIGH_WATERMARK_RATIO=0.0`)
- 200파일마다 모델 자동 언로드/리로드 (MPS 단편화 방지)
- 임베딩 결과 numpy 직접 전달 (메모리 7배 절감)
- OCR 페이지별 처리 (스캔 PDF 메모리 폭주 방지)

> **8GB Mac에서 인덱싱 시**: 다른 앱을 최대한 닫고 실행하세요. swap이 발생할 수 있지만 정상적으로 완료됩니다.
